...
public class FondoSepelioController {
	...
	private List<Tabla>			listaTipoTitular;
	...
	private List<Prevision> listaFondoSepelio;
	
	public FondoSepelioController(){
		log = Logger.getLogger(this.getClass());
		cargarUsuario();
		poseePermiso = PermisoUtil.poseePermiso(Constante.PARAM_TRANSACCION_REPORTE_FONDOSEPELIO);
	...
	public void cargarValoresIniciales(){
		try{
			...
			listaFondoSepelio = new ArrayList<Prevision>();
	...
	private void cargarListaTipoTitular(){
		List<Tabla> listaTipoVinculo= null;
		try {
			listaTipoVinculo = tablaFacade.getListaTablaPorAgrupamientoA(new Integer(Constante.PARAM_T_TIPOVINCULO), "A");
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		listaTipoTitular = listaTipoVinculo;
	}
	...
	public void buscar(){
		...
		try{
			listaFondoSepelio = previsionFacade.getListaFondoSepelio(
					Constante.PARAM_T_DOCUMENTOGENERAL_FONDOSEPELIO,
					intIdSucursal, 
					intIdSubSucursal, 
					intIdTipoFiltro, 
					strTipoFiltro, 
			...
			if(intEstadoSolicitud!=null && 
				...
				if(listaFondoSepelio!=null && !listaFondoSepelio.isEmpty()){
					for (Prevision fondoSepelio : listaFondoSepelio) {
						if(fondoSepelio.getStrEvaluacionEstado()==null){
							listaPrevisionTmp.add(fondoSepelio);
						}
					}
					listaFondoSepelio = listaPrevisionTmp;
				}
			}
			...
			if(intEstadoPago!=null && 
				...
				if(listaFondoSepelio!=null && !listaFondoSepelio.isEmpty()){
					for (Prevision fondoSepelio : listaFondoSepelio) {
						if(fondoSepelio.getStrEgresoFecha()==null){
							listaPrevisionTmp.add(fondoSepelio);
						}
					}
					listaFondoSepelio = listaPrevisionTmp;
	...
	public void imprimirFondoSepelio(){
		...
		try {
			...
			tipoVinculo = tablaFacade.getTablaPorIdMaestroYIdDetalle(new Integer(Constante.PARAM_T_TIPOVINCULO), intTipoTitular);
			...
			//estadoPago = tablaFacade.getTablaPorIdMaestroYIdDetalle(new Integer(Constante.PARAM_T_ESTADOSOLICPRESTAMO), intEstadoPago);
			...
			strNombreReporte = "prevision_fondoSepelio";
			UtilManagerReport.generateReport(strNombreReporte, parametro, 
					new ArrayList<Object>(listaFondoSepelio), Constante.PARAM_T_TIPOREPORTE_PDF);
		} catch (Exception e) {
	...
	public String getLimpiarFondoSepelio(){
		cargarUsuario();
		poseePermiso = PermisoUtil.poseePermiso(Constante.PARAM_TRANSACCION_REPORTE_FONDOSEPELIO);
	...
	public List<Tabla> getListaTipoTitular() {
		return listaTipoTitular;
	}

	public void setListaTipoTitular(List<Tabla> listaTipoTitular) {
	...
	public List<Prevision> getListaFondoSepelio() {
		return listaFondoSepelio;
	}

	public void setListaFondoSepelio(List<Prevision> listaFondoSepelio) {
		this.listaFondoSepelio = listaFondoSepelio;
	}
}
