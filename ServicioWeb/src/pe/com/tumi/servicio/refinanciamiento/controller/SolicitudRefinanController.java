...
//26.06.2014 JCHAVEZ Se procede a comentar codigo no utilizado(se comenta caso si se deba de utilizar en un futuro)
public class SolicitudRefinanController {
	 ...
	 * Clase Controlador
	 */
	public SolicitudRefinanController() {
	...
	private int isValidTodaSolicitud() {
		...
		try {
			...
			if (listaGarantiaCreditoComp != null
					&& listaGarantiaCreditoComp.size() <= 0 && !blnBtnAddGarante) {
				cnt++;
			}
...
//							EstadoCredito estadoCreditoSolicitud = beanExpedienteCredito.getListaEstadoCredito().get(maximo - 1);
						...
						beanSocioComp.getCuentaComp().setListExpedienteMovimientoComp(recuperarCreditosMovimiento());
						BigDecimal bdMontoInteresAtrasado = BigDecimal.ZERO;
						//jchavez 26.06.2014 en la opcion de ver no debe de calcular sino traer el interes ya grabado
						bdMontoInteresAtrasado = beanExpedienteCredito.getBdMontoInteresAtrasado();// calcularInteresAtrasado(beanExpedienteCreditoAnterior.getId());
						strInteresAtrasadoDelPrestamoRefinanciado = ""+bdMontoInteresAtrasado; 
						recalcularMontos();
						evaluarRefinanciamientoModificar2(event);
						blnPostEvaluacion = true;
						//cargando cronograma para su vizualizacion
						// recuperando datos a cronograma
						List<CronogramaCredito> cCred = boCronograma.getListaPorPkExpedienteCredito(this.beanExpedienteCredito.getId());
				        CronogramaCreditoComp cronCred = null;
				        listaCronogramaCreditoComp.clear();
				        Integer i = 0;
				        if ((cCred != null) && (!cCred.isEmpty())) {
							for (CronogramaCredito o : cCred) {
							    if (o.getIntParaTipoConceptoCod().equals(2)){
							    	cronCred = new CronogramaCreditoComp();
							    	log.info("------ visualizacion cronograma generado ------");
							    	cronCred.setStrFechaEnvio(o.getTsFechaEnvioView() != null 
							    		? sdfPeriodo.format(o.getTsFechaEnvioView()) 
							    		: "");
								    cronCred.setStrFechaVencimiento(o.getTsFechaVencimiento() != null 
								    	? sdfPeriodo.format(o.getTsFechaVencimiento()) 
								    	: "");
								  if (i.equals(0)) {
									  cronCred.setIntDiasTranscurridos((o.getTsFechaEnvioView() != null) && (o.getTsFechaVencimiento() != null) 
										?fechasDiferenciaEnDias(o.getTsFechaEnvioView(), o.getTsFechaVencimiento())
										:null);
								  } else {
									  cronCred.setIntDiasTranscurridos((o.getTsFechaEnvioView() != null) && (o.getTsFechaVencimiento() != null) 
										? (fechasDiferenciaEnDias(o.getTsFechaEnvioView(), o.getTsFechaVencimiento()) + 1) 
										: null);
								  }
								  cronCred.setBdSaldoCapital(o.getBdSaldoCapitalView());
								  cronCred.setBdAmortizacion(o.getBdAmortizacionView());
								  cronCred.setBdInteres(o.getBdInteresView());
								  cronCred.setBdCuotaMensual(o.getBdAmortizacionView().add(o.getBdInteresView()));
								  cronCred.setBdAportes(o.getBdAportesView());
								  cronCred.setBdTotalCuotaMensual((o.getBdAmortizacionView() != null) && (o.getBdInteresView() != null) && (o.getBdAportesView() != null) 
									    ? o.getBdAmortizacionView().add(o.getBdInteresView()).add(o.getBdAportesView()) 
									    : null);
								  
								  listaCronogramaCreditoComp.add(cronCred);
								  i++;
							    }
							}
				        }						
...
//		
		try {
			...
			if (beanSocioComp.getSocio().getSocioEstructura() != null) {
				...
				log.info("recuperamos las configuraciones de credito del antiguo prestamo..."+lstConfCreditos);
		...
		try {
			...
			if(confCreditoAntiguo != null){
				...
				if(confCreditoAntiguo.getIntParaTipoCreditoEmpresa() != null){
					if (confCreditoAntiguo.getId().getIntParaTipoCreditoCod().equals(Constante.PARAM_T_TIPO_CREDITO_REFINANCIADO)) {
						if (confCreditoAntiguo.getIntParaTipoCreditoEmpresa().equals(Constante.PARAM_T_TIPOCREDITOEMPRESA_REFINANCIAMIENTO_ORDINARIO)) {
							intTipoCreditoEmpresa = Constante.PARAM_T_TIPOCREDITOEMPRESA_ORDINARIO;
						}
						if (confCreditoAntiguo.getIntParaTipoCreditoEmpresa().equals(Constante.PARAM_T_TIPOCREDITOEMPRESA_REFINANCIAMIENTO_CUBIERTO)) {
							intTipoCreditoEmpresa = Constante.PARAM_T_TIPOCREDITOEMPRESA_CUBIERTO;
						}
						if (confCreditoAntiguo.getIntParaTipoCreditoEmpresa().equals(Constante.PARAM_T_TIPOCREDITOEMPRESA_REFINANCIAMIENTO_PROMOCIONAL)) {
							intTipoCreditoEmpresa = Constante.PARAM_T_TIPOCREDITOEMPRESA_PROMOCIONAL;
						}
						if (confCreditoAntiguo.getIntParaTipoCreditoEmpresa().equals(Constante.PARAM_T_TIPOCREDITOEMPRESA_REFINANCIAMIENTO_ESPECIAL)) {
							intTipoCreditoEmpresa = Constante.PARAM_T_TIPOCREDITOEMPRESA_ESPECIAL;
						}
						if (confCreditoAntiguo.getIntParaTipoCreditoEmpresa().equals(Constante.PARAM_T_TIPOCREDITOEMPRESA_REFINANCIAMIENTO_ADMINISTRATIVO)) {
							intTipoCreditoEmpresa = Constante.PARAM_T_TIPOCREDITOEMPRESA_ADMINISTRATIVO;
						}
					}else{
...
//		boolean boFlag = false;
		...
		try {
			...
			if (beanSocioComp.getSocio().getSocioEstructura() != null) {
				...
				if (beanExpedienteMovARefinanciar==null) {
					return;
				}
...
//										Integer intModalidad = beanSocioComp.getSocio().getSocioEstructura().getIntModalidad();
			...
			if(!validarMontosCapacidadesCredito(bdTotalCuotaMensual)){
				blnPostEvaluacion = false;
						...
						if (beanSocioComp.getPersona().getPersonaEmpresa().getListaPersonaRol() != null) {
							...
							if(blnRol){
								...
								for (int k = 0; k < beanSocioComp.getPersona().getNatural().getListaPerLaboral().size(); k++) {
									//AGREGADO 19.03.2014 JCHAVEZ VALIDACION "TODOS"
									if (confRefinan.getIntParaCondicionLaboralCod().equals(-1)) {
										boFlagCondLab = true;
										break;
									}
 ...
 * @return listaEnvioVencimiento (0, fec1erEnvio)(1, fec1erVenc);
 */
	//jchavez 26.06.2014 Codigo no utilizado
...
//		GarantiaCreditoComp garantiaCreditoCompTmp = null;
		List<GarantiaCreditoComp> listaGarantiaCreditoCompTemp = new ArrayList<GarantiaCreditoComp>();
		try {
			if (listaGarantiaCreditoComp != null) {
...
//						break;
						...
						listaGarantiaCreditoCompTemp.add(listaGarantiaCreditoComp.get(i));
					}
				}
				//26.06.2014 JCHAVEZ Codigo no utilizado
...
//				}
				listaGarantiaCreditoComp.clear();
				listaGarantiaCreditoComp.addAll(listaGarantiaCreditoCompTemp);
			}
		} catch (Exception e) {
			log.error("error: " + e);
		}
...
//		ExpedienteComp expedienteComp = null;
		try {
			//AGREGADO 27.03.2014
			registroSeleccionadoBusqueda = (ExpedienteCreditoComp)event.getComponent().getAttributes().get("item");
 ...
 */
    ...
    public void imprimirSolicitudPrestamo(){
		...
		try {
	        ...
	        //CONTRATO DE PRESTAMO
	        //parametro.put("P_NOMYAPELLIDO", beanSocioComp.getPersona().getNatural().getStrNombres() +","+ beanSocioComp.getPersona().getNatural().getStrApellidoPaterno() +""+ beanSocioComp.getPersona().getNatural().getStrApellidoMaterno());
	        //parametro.put("P_INTERES", beanExpedienteCredito.getBdPorcentajeInteres()!=null?beanExpedienteCredito.getBdPorcentajeInteres():" ");
...
//			parametro.put("P_NUMEROCHEQUE", beanExpedienteCredito.getEgreso().getIntNumeroCheque()); 
	        ...
	        parametro.put("P_MONTOENLETRAS", letras);
	        System.out.println("MONTO LETRAS " + letras);
	        listaSucursal = empresaFacade.getListaSucursalPorPkEmpresa(Constante.PARAM_EMPRESASESION);
    ...
    public void imprimirPagare(){
		...
		try {
			...
			if(beanCredito.getBdTasaMoratoriaMensual()!=null)
				parametro.put("P_TASAMENSUAL", beanCredito.getBdTasaMoratoriaMensual());
			else
...
//			            	strVia1 = tablaVia1.getStrDescripcion();
						...
						parametro.put("P_DIRECCIONGARANTE2", strVia+" "+(personaGarante.getListaDomicilio().get(0).getStrNombreVia()!=null?personaGarante.getListaDomicilio().get(0).getStrNombreVia():"")
															+ " " + (personaGarante.getListaDomicilio().get(0).getIntNumeroVia()!=null?personaGarante.getListaDomicilio().get(0).getIntNumeroVia():"")
															+ " " +(personaGarante.getListaDomicilio().get(0).getStrInterior()!=null?personaGarante.getListaDomicilio().get(0).getStrInterior():""));
						parametro.put("P_DIRECCIONGARANTE3", " ");
						parametro.put("P_NOMBREGARANTE2", personaGarante.getNatural().getStrApellidoPaterno().toUpperCase() 
...
*/
	...
	public Boolean getBlnBtnAddGarante() {
