...
public class SolicitudPrestamoServiceV2 {
				...
				for(ExpedienteCredito expedienteCredito : listaExpedienteCredito){
					...
					if(creditoRec != null){
						...
						if(!listaDescripcionExpedienteXredito.isEmpty()){
							...
							//Tabla tabDesc = null;
							//tabDesc = listaDescripcionExpedienteXredito.get(0);
...
//	}
	 ...
	 * @throws BusinessException
	 */
//	 private void generarTransferencia(SolicitudCtaCteTipo solicitudCtaCteTipo) throws BusinessException{
//		 PersonaFacadeRemote       	personaFacade     = null;
...
//			      transferencia.setTsTranFecha(new Timestamp(new Date().getTime()));
...
//			      cuentaCteFacade.grabarTransgerencia(transferencia) ;
...
//		    	
//	      }catch(BusinessException e){
//	    	  log.error("Error - BusinessException - en debug generarTransferencia --> "+e);
//				throw e;
//		  }catch(Exception e){
//			  log.error("Error - Exception - en debug generarTransferencia --> "+e);
//				throw new BusinessException(e);
//		  }
//	 }
//	 
...
//				 }else{
//					// No existe modelo de refinanciamiento 
//				 }
//
//				 if (listaModelo == null || listaModelo.size() == 0){
//					  throw new BusinessException("Error. No existe el modelo contable para generar el asiento.");
//				 }
//	             
...
//				 diario.setStrGlosa("Lib Diario - Transferencia de refinanciamiento...");
...
//				 diario.setIntParaTipoDocumentoGeneral(Constante.PARAM_T_DOCUMENTOGENERAL_PRESTAMOS);
...
//						id.setIntContCodigoLibro(diarioResult.getId().getIntContCodigoLibro());
//						id.setIntContPeriodoLibro(diarioResult.getId().getIntContPeriodoLibro());
//						id.setIntPersEmpresaLibro(diarioResult.getId().getIntPersEmpresaLibro());
...
//									if (modeloDet == null){
//											  throw new BusinessException("No existe el Número de Cuenta en el Modelo Contable(CodModelo:"+modelo.getId().getIntCodigoModelo()+
//													                      ",intTipoCategRiesgo = "+intTipoCategRiesgo+",intMiniValor = "+intValorFijo+",intTipoCptoGeneral="+intTipoCptoGeneral+"intTipoCreditoEmpresa="+intTipoCreditoEmpresa+")");
//									}
//							    }	
//									
//								diarioDetalle.setIntPersEmpresaCuenta(modeloDet.getId().getIntEmpresaPk());
//								diarioDetalle.setIntContPeriodo(modeloDet.getId().getIntContPeriodoCuenta());
...
//							    
//							    if (intValorFijo != null && intValorFijo.equals(Constante.PARAM_T_TIPOCREDITO_TOTAL_TRANSFERENCIA)){
...
//						
//								//Se actualiza los saldos de Prestamo,Credito y Mora
...
//										   BigDecimal bdSaldoCredito = expediente.getBdSaldoCredito().subtract(expediente.getBdMontoAbono() != null?expediente.getBdMontoAbono():new BigDecimal(0));
//										   expediente.setBdSaldoCredito(bdSaldoCredito);
//									   }
//									   else
//									   if(expediente.getStrDescripcion() != null && expediente.getStrDescripcion().equalsIgnoreCase("Interes")){
//										   BigDecimal bdSaldoInteres = expediente.getBdSaldoCredito().subtract(expediente.getBdMontoAbono() != null?expediente.getBdMontoAbono():new BigDecimal(0));
//										   expediente.setBdSaldoInteres(bdSaldoInteres);
//									   }
//									   else
//									   if(expediente.getStrDescripcion() != null && expediente.getStrDescripcion().equalsIgnoreCase("Mora")){
//										   BigDecimal bdSaldoMora = expediente.getBdSaldoCredito().subtract(expediente.getBdMontoAbono() != null?expediente.getBdMontoAbono():new BigDecimal(0));
//										   expediente.setBdSaldoMora(bdSaldoMora);
//							    }
//									   
...
//		   
//	      }catch(BusinessException e){
//	    	  log.error("Error - BusinessException - en debug generarAsientoContable --> "+e);
//				throw e;
//		  }catch(Exception e){
//			  log.error("Error - Exception - en debug generarAsientoContable --> "+e);
//				throw new BusinessException(e);
//		  }
//	 }
...
//		           String  mesTransf     = obtieneMesCadena(fechaTrans);
//		       	   String  anioTransf    = obtieneAnioCadena(fechaTrans);
...
//		          		  String  mesCron         = obtieneMesCadena(cronograma.getTsFechaVencimiento());
//		           		  String  anioCron        = obtieneAnioCadena(cronograma.getTsFechaVencimiento());
...
//		 
//	   }catch(BusinessException e){
//		   log.error("Error - BusinessException - en debug actualizaMontoSaldoCronogramaDelSocio --> "+e);
//			throw e;
//	   }catch(Exception e){
//		   log.error("Error - Exception - en debug actualizaMontoSaldoCronogramaDelSocio --> "+e);
//			throw new BusinessException(e);
//	   }
//	}
// 
...
////			            }  
////			        });  
//					Collections.sort(lista, new Comparator<Cronograma>(){
//						public int compare(Cronograma o1, Cronograma o2) {
//							return o1.getIntNumeroCuota().compareTo(o2.getIntNumeroCuota());
//						}
//					});
//		          	  
...
//		          	for (Cronograma cronograma : lista) {
//		          		 if (cronograma.getIntParaTipoConceptoCreditoCod().equals(tipConceptoGeneral)){
//		          			  if (montoAbono.compareTo(new BigDecimal(0)) == 0){
...
//			  }
//	   }catch(BusinessException e){
//		   log.error("Error - BusinessException - en debug actualizaMontoSaldoCronoPorCoutas --> "+e);
//			throw e;
//	   }catch(Exception e){
//		   log.error("Error - Exception - en debug actualizaMontoSaldoCronoPorCoutas --> "+e);
//			throw new BusinessException(e);
//	   }
//	}
//	
	 ...
	 * @throws BusinessException
	 */
//	private void actualizarMontoAporteDelSocioTitularOGarante(Expediente exp, SolicitudCtaCteTipo solCtaCteTipo ) throws BusinessException{
//		
//			ConceptoFacadeRemote conceptoFacade = null;
...
//			
//			}catch(BusinessException e){
//				log.error("Error - BusinessException - en debug actualizarMontoAporteDelSocioTitularOGarante --> "+e);
//				throw e;
//			}catch(Exception e){
//				log.error("Error - Exception - en debug actualizarMontoAporteDelSocioTitularOGarante --> "+e);
//				throw new BusinessException(e);
//			}
//		}
//			 
	 ...
	 * @param intEmpresa
	 * @param intCuenta
	 * @param intPersonaTitularOGarante
	 * @param intEmpresaTitularOGarante
	 * @param intCuentaTitularOGarante
	 * @throws BusinessException
	 */
//	private void generarMovimientoParaCadaTipoCptoGnral(Expediente expediente, SolicitudCtaCteTipo solicitudCtaCteTipo,
//         Integer intPersona,  Integer intEmpresa, Integer intCuenta, Integer intPersonaTitularOGarante, Integer intEmpresaTitularOGarante,
...
//				mov.setTsFechaMovimiento(obtieneFechaActualEnTimesTamp());
...
//					mov.setBdMontoMovimiento(expediente.getBdSaldoCredito());
//					conceptoFacade.grabarMovimiento(mov);
//				}
//			}else{
...
//		}
//		catch(BusinessException e){
//			log.error("Error - BusinessException - en debug generarMovimientoParaCadaTipoCptoGnral --> "+e);
//			throw e;
//		}catch(Exception e){
//			log.error("Error - Exception - en debug generarMovimientoParaCadaTipoCptoGnral --> "+e);
//			throw new BusinessException(e);
//			}
//		}
//			 
	...
	/*
		modeloDet = obtieneElCodigoAsientoyNroCtaDelPlanCuentaActual(modelo.getId(),intTipoCategRiesgo,
												                 	valorFijo, intTipoCptoGeneral,
												                 	intParaTipoConcepto, intTipoCreditoEmpresa);
	 */
	
		/**
		 * 
		 */
//		private ModeloDetalle obtieneElCodigoAsientoyNroCtaDelPlanCuentaActual(ModeloId idModelo, Integer  intTipoCategRiesgo,  Integer  valorFijo, 
//			Integer  intTipoCptoGeneral, Integer  intParaTipoConcepto, Integer  intTipoCreditoEmpresa) throws BusinessException{
...
//								break;
//							}
//							else if (modeloDetalleNivel.getIntDatoTablas()    != null && modeloDetalleNivel.getIntDatoTablas().equals(new Integer(Constante.PARAM_T_TIPOCONCEPTOGENERAL)) &&
...
//								modeloDetalleResult = modeloDetalle;
//							}
//							else if (intTipoCptoGeneral != null && intParaTipoConcepto != null){
...
//
//			}catch(BusinessException e){
//				log.error("Error - BusinessException - en obtieneElCodigoAsientoyNroCtaDelPlanCuentaActual --> "+e);
//				throw e;
//			}catch(Exception e){
//				log.error("Error - Exception - en obtieneElCodigoAsientoyNroCtaDelPlanCuentaActual --> "+e);
//				throw new BusinessException(e);
//			}
//			
...
//			 
//		   }catch(BusinessException e){
//			   log.error("Error - BusinessException - en actualizaMontoSaldoCronogramaEnCero --> "+e);
//				throw e;
//		   }catch(Exception e){
//			   log.error("Error - Exception - en actualizaMontoSaldoCronogramaEnCero --> "+e);
//				throw new BusinessException(e);
//		   }
//		}
//	 
		...
		public LibroDiario generarProcesoAutorizacionRefinanciamientoTotal(ExpedienteCredito expedienteCredito, Integer intParaEstadoAprobado, Integer intParaTipoOperacion,
				ActionEvent event, Usuario usuario,  SocioComp beanSocioComp, ExpedienteCreditoComp expedienteCreditoCompSelected, String strPeriodo,  
				RequisitoCredito requisitoCred ,  Expediente expedientePorRefinanciar) throws BusinessException{
				
				LibroDiario libroDiario = null;
				
				try {
					//1. GENERAMOS LA SOLICITUD DE  CTA CTE
						libroDiario = generarSolicitudCtaCteRefinan(beanSocioComp, usuario, strPeriodo, requisitoCred, expedientePorRefinanciar, expedienteCredito);
					
						//2.GENERAR EL EXPEDIENTE DE MOVIMIENTO del prestamo refinanciado (2)
						generarExpedienteMovimiento(expedienteCreditoCompSelected);
					
						//3 CAMBIO DE ESTADO DE EXPEDIENTE POR REFINANCIAR (2)- en servicio
						cambioEstadoExpedientePorRefinanciar(usuario, beanSocioComp, expedienteCreditoCompSelected);
					
						//4. CAMBIO DE ESTADO DE 	
						//cambioEstadoCredito(expedienteCredito, intParaEstadoAprobado, usuario);
					
						//5. se graba los datos de la autorizacion
						//grabarAutorizacionPrestamo(expedienteCredito);
				} catch (Exception e) {
		...
		*/
		...
		/**
		 * Cambia el estado del expediente credito anterior y 
		 * @throws BusinessException
		 * @throws Exception 
		 */
//		private void cambioEstadoExpedientePorRefinanciar(Usuario usuario, SocioComp beanSocioComp, ExpedienteCreditoComp expedienteCreditoCompSelected) throws BusinessException, Exception{
//			ConceptoFacadeRemote conceptofacade = null;
//			SolicitudPrestamoFacadeRemote solicitudPrestamoFacade = null;
//			ExpedienteCreditoId idCreditoAnterior = null;
//			try {
//				
//				conceptofacade = (ConceptoFacadeRemote)EJBFactory.getRemote(ConceptoFacadeRemote.class);
//				solicitudPrestamoFacade = (SolicitudPrestamoFacadeRemote)EJBFactory.getRemote(SolicitudPrestamoFacadeRemote.class);
//				
//				ExpedienteCredito exp = expedienteCreditoCompSelected.getExpedienteCredito();
//				ExpedienteCredito expedienteAnterior = null;
//				Expediente expedienteMovimiento = null;
////				List<Movimiento> lstMovimiento = null;
//				
//				Integer intDetExpAnt = exp.getId().getIntItemDetExpediente();
//				intDetExpAnt--;
//
//				idCreditoAnterior = new ExpedienteCreditoId();
//				idCreditoAnterior.setIntCuentaPk(exp.getId().getIntCuentaPk());
//				idCreditoAnterior.setIntItemDetExpediente(intDetExpAnt);
//				idCreditoAnterior.setIntItemExpediente(exp.getId().getIntItemExpediente());
//				idCreditoAnterior.setIntPersEmpresaPk(exp.getId().getIntPersEmpresaPk());
//				
//				
//				expedienteAnterior = solicitudPrestamoFacade.getSolicitudPrestamoPorIdExpedienteCredito(idCreditoAnterior);
//				if(expedienteAnterior != null){
//					EstadoCredito estadoFinal = new EstadoCredito();
//					EstadoCreditoId id = new EstadoCreditoId();
//
//					id.setIntCuentaPk(expedienteAnterior.getId().getIntCuentaPk());
//					id.setIntItemDetExpediente(expedienteAnterior.getId().getIntItemDetExpediente());
//					id.setIntItemEstado(null);
//					id.setIntItemExpediente(expedienteAnterior.getId().getIntItemExpediente());
//					id.setIntPersEmpresaPk(expedienteAnterior.getId().getIntPersEmpresaPk());
//					estadoFinal.setId(id);
//					
//					estadoFinal.setTsFechaEstado(new Timestamp(new Date().getTime()));
//					estadoFinal.setIntPersEmpresaEstadoCod(usuario.getEmpresa().getIntIdEmpresa());
//					estadoFinal.setIntIdUsuSucursalPk(usuario.getSucursal().getId().getIntIdSucursal());
//					estadoFinal.setIntIdUsuSubSucursalPk(usuario.getSubSucursal().getId().getIntIdSubSucursal());
//					estadoFinal.setIntPersUsuarioEstadoPk(usuario.getIntPersPersonaPk());
//					estadoFinal.setIntParaEstadoCreditoCod(Constante.PARAM_T_ESTADOSOLICPRESTAMO_CANCELADO);
//					
//					//expedienteAnterior.getListaEstadoCredito().add(estadoFinal);
//					estadoFinal = solicitudPrestamoFacade.grabarEstadoAutorizadoRefinan(estadoFinal);
//					
//					//---------------------------------------------------------------------------------
//					
//					//Integer intPersEmpresa =  expedienteAnterior.getId().getIntPersEmpresaPk();
//					//Integer intCuenta = expedienteAnterior.getId().getIntCuentaPk() ;
//					//Integer intItemExpediente =  expedienteAnterior.getId().getIntItemExpediente();
//					//Integer intItemExpedienteDetalle = expedienteAnterior.getId().getIntItemDetExpediente() ;
//					
//					ExpedienteId expId = new ExpedienteId();
//					expId.setIntCuentaPk(expedienteAnterior.getId().getIntCuentaPk());
//					expId.setIntItemExpediente(expedienteAnterior.getId().getIntItemExpediente());
//					expId.setIntItemExpedienteDetalle(expedienteAnterior.getId().getIntItemDetExpediente());
//					expId.setIntPersEmpresaPk(expedienteAnterior.getId().getIntPersEmpresaPk());
//
//					expedienteMovimiento = conceptofacade.getExpedientePorPK(expId);
//					// Integer intPersEmpresa,Integer intCuenta,Integer intItemExpediente, Integer intItemExpedienteDetalle
//					
//					if(expedienteMovimiento != null){
//						
//						//lstMovimiento = conceptofacade.getListXCtaExpediente(intPersEmpresa, intCuenta, intItemExpediente, intItemExpedienteDetalle);
//						//conceptofacade.getListaMovimientoPorCuentaEmpresaConcepto(arg0, arg1, arg2)
//						//if(lstMovimiento != null){
//							//boolean blnExisteAmortizacion = Boolean.FALSE;
//							//boolean blnExisteInteres = Boolean.FALSE;
//							//boolean blnExisteMora = Boolean.FALSE;
//							Movimiento movAmortizacion = null;
//							Movimiento movInteres = null;
//							Movimiento movMora = null;
//							Movimiento movimientoMaximo = null;
//							
//							/*for (Movimiento movimiento : lstMovimiento) {
//								if( movimiento.getIntParaTipoConceptoGeneral().compareTo(Constante.PARAM_T_TIPOCONCEPTOPRESTAMO_AMORTIZACION)==0){
//									blnExisteAmortizacion = Boolean.TRUE;
//									movAmortizacion = movimiento;
//								}else if ( movimiento.getIntParaTipoConceptoGeneral().compareTo(Constante.PARAM_T_TIPOCONCEPTOPRESTAMO_INTERES)==0){
//											blnExisteInteres = Boolean.TRUE;
//											movInteres = movimiento;		
//								}else if ( movimiento.getIntParaTipoConceptoGeneral().compareTo(Constante.PARAM_T_TIPOCONCEPTOPRESTAMO_MORA)==0){
//											blnExisteMora = Boolean.TRUE;
//											movMora =  movimiento;				
//								}
//							}*/
//							
//							// Se genera o mdifica si existe o no...
//							movimientoMaximo = conceptofacade.getMaximoMovimiento();
//							Integer intItem = movimientoMaximo.getIntItemMovimiento()  +1 ;
//							
//							movAmortizacion = new Movimiento();
//							movAmortizacion.setIntItemMovimiento(intItem);
//							movAmortizacion.setTsFechaMovimiento(new Timestamp(new Date().getTime()));
//							movAmortizacion.setIntCuenta(expedienteAnterior.getId().getIntCuentaPk());
//							movAmortizacion.setIntPersPersonaIntegrante(beanSocioComp.getCuenta().getIntIdPersona());
//							movAmortizacion.setIntItemExpediente(exp.getId().getIntItemExpediente());
//							movAmortizacion.setIntItemExpedienteDetalle(exp.getId().getIntItemDetExpediente());
//							movAmortizacion.setIntParaTipoConceptoGeneral(Constante.PARAM_T_TIPOCONCEPTOPRESTAMO_AMORTIZACION);
//							movAmortizacion.setIntParaTipoMovimiento(Constante.PARAM_T_FORMADEPAGO_TRANSFERENCIACONTABLE);
//							movAmortizacion.setIntParaDocumentoGeneral(Constante.PARAM_T_DOCUMENTOGENERAL_TX_ENTERCONCEPTOS);//
//							movAmortizacion.setStrNumeroDocumento(exp.getId().getIntItemExpediente()+"-"+exp.getId().getIntItemDetExpediente());
//							movAmortizacion.setBdMontoMovimiento(expedienteMovimiento.getBdMontoSaldoDetalle() == null ? BigDecimal.ZERO :expedienteMovimiento.getBdMontoSaldoDetalle() );
//							movAmortizacion.setBdMontoSaldo(BigDecimal.ZERO);
//							movAmortizacion.setIntPersEmpresa(usuario.getEmpresa().getIntIdEmpresa());
//							movAmortizacion.setIntPersEmpresaUsuario(usuario.getIntPersPersonaPk());
//							movAmortizacion.setIntPersPersonaIntegrante(beanSocioComp.getPersona().getIntIdPersona());
//							movAmortizacion = conceptofacade.grabarMovimiento(movAmortizacion);
//							System.out.println(movAmortizacion.getIntItemCuentaConcepto());
//							
//							System.out.println("=============================================");
//							
//							movimientoMaximo = conceptofacade.getMaximoMovimiento();
//							Integer intItem2 = movimientoMaximo.getIntItemMovimiento()  +1 ;
//							
//							movInteres = new Movimiento();
//							movInteres.setIntItemMovimiento(intItem2);
//							movInteres.setTsFechaMovimiento(new Timestamp(new Date().getTime()));
//							movInteres.setIntCuenta(expedienteAnterior.getId().getIntCuentaPk());
//							movInteres.setIntPersPersonaIntegrante(beanSocioComp.getCuenta().getIntIdPersona());
//							movInteres.setIntItemExpediente(exp.getId().getIntItemExpediente());
//							movInteres.setIntItemExpedienteDetalle(exp.getId().getIntItemDetExpediente());
//							movInteres.setIntParaTipoConceptoGeneral(Constante.PARAM_T_TIPOCONCEPTOPRESTAMO_INTERES);
//							movInteres.setIntParaTipoMovimiento(Constante.PARAM_T_FORMADEPAGO_TRANSFERENCIACONTABLE);
//							movInteres.setIntParaDocumentoGeneral(Constante.PARAM_T_DOCUMENTOGENERAL_TX_ENTERCONCEPTOS);//
//							movInteres.setStrNumeroDocumento(exp.getId().getIntItemExpediente()+"-"+exp.getId().getIntItemDetExpediente());
//							movInteres.setBdMontoMovimiento(expedienteMovimiento.getBdMontoInteresAtrazado() == null ? BigDecimal.ZERO :expedienteMovimiento.getBdMontoInteresAtrazado() );
//							movInteres.setBdMontoSaldo(BigDecimal.ZERO);
//							movInteres.setIntPersEmpresa(usuario.getEmpresa().getIntIdEmpresa());
//							movInteres.setIntPersEmpresaUsuario(usuario.getIntPersPersonaPk());
//							movInteres.setIntPersPersonaIntegrante(beanSocioComp.getPersona().getIntIdPersona());
//							movInteres = conceptofacade.grabarMovimiento(movInteres);
//							System.out.println(movInteres.getIntItemCuentaConcepto());
//							System.out.println("=============================================");
//							
//							movimientoMaximo = conceptofacade.getMaximoMovimiento();
//							Integer intItem3 = movimientoMaximo.getIntItemMovimiento()  +1 ;
//							
//							movMora = new Movimiento();
//							movMora.setIntItemMovimiento(intItem3);
//							movMora.setTsFechaMovimiento(new Timestamp(new Date().getTime()));
//							movMora.setIntCuenta(expedienteAnterior.getId().getIntCuentaPk());
//							movMora.setIntPersPersonaIntegrante(beanSocioComp.getCuenta().getIntIdPersona());
//							movMora.setIntItemExpediente(exp.getId().getIntItemExpediente());
//							movMora.setIntItemExpedienteDetalle(exp.getId().getIntItemDetExpediente());
//							movMora.setIntParaTipoConceptoGeneral(Constante.PARAM_T_TIPOCONCEPTOPRESTAMO_MORA);
//							movMora.setIntParaTipoMovimiento(Constante.PARAM_T_FORMADEPAGO_TRANSFERENCIACONTABLE);
//							movMora.setIntParaDocumentoGeneral(Constante.PARAM_T_DOCUMENTOGENERAL_TX_ENTERCONCEPTOS);//
//							movMora.setStrNumeroDocumento(exp.getId().getIntItemExpediente()+"-"+exp.getId().getIntItemDetExpediente());
//							movMora.setBdMontoMovimiento(expedienteMovimiento.getBdMontoMoraAtrazado() == null ? BigDecimal.ZERO :expedienteMovimiento.getBdMontoMoraAtrazado() );
//							movMora.setBdMontoSaldo(BigDecimal.ZERO);
//							movMora.setIntPersEmpresa(usuario.getEmpresa().getIntIdEmpresa());
//							movMora.setIntPersEmpresaUsuario(usuario.getIntPersPersonaPk());
//							movMora.setIntPersPersonaIntegrante(beanSocioComp.getPersona().getIntIdPersona());
//							movMora = conceptofacade.grabarMovimiento(movMora);
//							System.out.println(movMora.getIntItemCuentaConcepto());
//							System.out.println("=============================================");
//							
//						//}
//					}
...
//
//			} catch (BusinessException e) {
//				log.error("Error - BusinessException - en cambioEstadoExpedientePorRefinanciar()"+e);
//				throw new BusinessException(e);
//				
//			}
//		}		
...
//		ExpedienteActividad expAct = null;
		...
		Modelo modeloContable = null;
//		Movimiento movimientoRefinanciamiento = null;
...
//			carterafacade = (CarteraFacadeRemote)EJBFactory.getRemote(CarteraFacadeRemote.class);
			...
			if(modeloContable != null){
				...
				if(listaModDet != null && !listaModDet.isEmpty()){
					...
					diario.setId(new LibroDiarioId());
					diario.getId().setIntContPeriodoLibro(new Integer(anio+""+mes));
					diario.getId().setIntPersEmpresaLibro(expedienteCreditoRef.getId().getIntPersEmpresaPk());
					diario.setStrGlosa("Asiento de Autorizacion de Refinanciamiento. ");
					diario.setTsFechaRegistro(new Timestamp((new Date()).getTime())); //la fecha actual
					diario.setTsFechaDocumento(new Timestamp((new Date()).getTime()));
					diario.setIntParaTipoDocumentoGeneral(Constante.PARAM_T_DOCUMENTOGENERAL_ACTIVIDAD);
					diario.setIntParaEstado(Constante.PARAM_T_ESTADOUNIVERSAL_ACTIVO);
					...
					//----------------------------------------------------------------------------------------------
					// Recuperamos Datos del credito viejo
					...
					// recuperanod el tipo credito empresa.
					CreditoId creditoId= new CreditoId();
					...
					// Recuperamos el tipo credito
//					if(carteraCredDet != null){

						for (ModeloDetalle modeloDetalle : listaModDet) {
							bdValorColumna= BigDecimal.ZERO;
							Boolean blnValorFijo = Boolean.FALSE;
							Boolean blnTipoCredito = Boolean.FALSE;
							Boolean blnTipoRiesgo = Boolean.FALSE;
							...
							if (modeloDetalle.getId().getIntContPeriodoCuenta().equals(intPeriodoCuenta)){
								...
								if(listaModDetNiv != null && !listaModDetNiv.isEmpty()){
									// Niveles del detalle modelo
									for (ModeloDetalleNivel modeloDetalleNivel : listaModDetNiv) {
										// Es valor fijo, es decir que es el nombre del campo que se utilizará para el registro
										if(modeloDetalleNivel.getIntParaTipoRegistro().compareTo(Constante.PARAM_T_MODELO_TIPO_REGISTRO_FIJO)==0){
											if(modeloDetalleNivel.getStrDescripcion().equalsIgnoreCase(Constante.PARAM_T_MODELO_REFINANCIAMIENTO_SALDOCREDITO)
												&& modeloDetalle.getIntParaOpcionDebeHaber().compareTo(Constante.PARAM_T_OPCIONDEBEHABER_DEBE)==0){
													if (expedienteCreditoRef.getBdMontoTotal()!=BigDecimal.ZERO) {
														bdValorColumna = expedienteCreditoRef.getBdMontoTotal();	
														blnValorFijo = Boolean.TRUE;
													}													
											}else if(modeloDetalleNivel.getStrDescripcion().equalsIgnoreCase(Constante.PARAM_T_MODELO_REFINANCIAMIENTO_MONTOTOTAL)){
													if (expedienteCreditoRef.getBdMontoTotal()!=BigDecimal.ZERO) {
														bdValorColumna = expedienteCreditoRef.getBdMontoTotal();	
														blnValorFijo = Boolean.TRUE;
													}
											}else if(modeloDetalleNivel.getStrDescripcion().equalsIgnoreCase(Constante.PARAM_T_MODELO_REFINANCIAMIENTO_SALDOCREDITO)
													&& modeloDetalle.getIntParaOpcionDebeHaber().compareTo(Constante.PARAM_T_OPCIONDEBEHABER_HABER)==0){
													if (expedienteCreditoViejoMovimiento.getBdSaldoCredito()!=BigDecimal.ZERO) {
														bdValorColumna = expedienteCreditoViejoMovimiento.getBdSaldoCredito();
														blnValorFijo = Boolean.TRUE;
													}
											}else if(modeloDetalleNivel.getStrDescripcion().equalsIgnoreCase(Constante.PARAM_T_MODELO_REFINANCIAMIENTO_MONTOINTERESATRSADO)){
													if (expedienteCreditoRef.getBdMontoInteresAtrasado()!=BigDecimal.ZERO) {
														bdValorColumna = expedienteCreditoRef.getBdMontoInteresAtrasado();	
														blnValorFijo = Boolean.TRUE;
													}
											}else if(modeloDetalleNivel.getStrDescripcion().equalsIgnoreCase(Constante.PARAM_T_MODELO_REFINANCIAMIENTO_MONTOMORAATRASADA)){
													if (expedienteCreditoRef.getBdMontoMoraAtrasada()!=BigDecimal.ZERO) {
														bdValorColumna = expedienteCreditoRef.getBdMontoMoraAtrasada();	
														blnValorFijo = Boolean.TRUE;
													}
											}else if(modeloDetalleNivel.getStrDescripcion().equalsIgnoreCase(Constante.PARAM_T_MODELO_REFINANCIAMIENTO_MONTOSOLICITADO)){
													if (expedienteCreditoRef.getBdMontoSolicitado()!=BigDecimal.ZERO) {
														bdValorColumna = expedienteCreditoRef.getBdMontoSolicitado();	
														blnValorFijo = Boolean.TRUE;
													}
											}
										}

										// Es tabla, esto quiere decir que si elvalor del campo que se especifica, es igual al valor del mismo campo en el expediente
										// que se está trabajando.
										else if(modeloDetalleNivel.getIntParaTipoRegistro().compareTo(Constante.PARAM_T_MODELO_TIPO_REGISTRO_TABLA)==0){
											if(modeloDetalleNivel.getIntDatoTablas().compareTo(new Integer(Constante.PARAM_T_TIPOCREDITOEMPRESA))==0
													&& modeloDetalle.getIntParaOpcionDebeHaber().compareTo(Constante.PARAM_T_OPCIONDEBEHABER_HABER)==0){
												if(modeloDetalleNivel.getIntDatoArgumento().compareTo(confCreditoViejo.getIntParaTipoCreditoEmpresa())==0
													&& bdValorColumna != null){
													blnTipoCredito = Boolean.TRUE;
												}
											}
											if(modeloDetalleNivel.getIntDatoTablas().compareTo(new Integer(Constante.PARAM_T_TIPOCREDITOEMPRESA))==0
													&& modeloDetalle.getIntParaOpcionDebeHaber().compareTo(Constante.PARAM_T_OPCIONDEBEHABER_DEBE)==0
													&& bdValorColumna != null){
												if(modeloDetalleNivel.getIntDatoArgumento().compareTo(confCreditoNuevo.getIntParaTipoCreditoEmpresa())==0
													&& bdValorColumna != null){
													blnTipoCredito = Boolean.TRUE;
													blnTipoRiesgo = Boolean.TRUE;
												}
											}
											
											if(modeloDetalleNivel.getIntDatoTablas().compareTo(new Integer(Constante.PARAM_T_TIPOCATEGORIADERIESGO))==0){
												if(modeloDetalleNivel.getIntDatoArgumento().compareTo(carteraCredDet!=null?carteraCredDet.getIntParaTipocategoriariesgo():Constante.PARAM_T_TIPOCATEGORIADERIESGO_NORMAL)==0
													&& bdValorColumna != null){
													blnTipoRiesgo = Boolean.TRUE;
												}
											}											
									...
									if(blnValorFijo && blnTipoCredito && blnTipoRiesgo){
										diarioDet.setIntParaMonedaDocumento(Constante.PARAM_T_TIPOMONEDA_SOLES);
										diarioDet.setIntContPeriodo(modeloDetalle.getId().getIntContPeriodoCuenta());
										diarioDet.setIntPersEmpresaCuenta(modeloDetalle.getId().getIntPersEmpresaCuenta());
										diarioDet.setIntPersPersona(usuario.getIntPersPersonaPk());
						...
						if(diario.getId() != null){ // expedienteCreditoAut
							...
							cambioEstadoActividad(expedienteCreditoAut, usuario);
						}else{
		...
		// 1. MOVIMIENTO: crear prestamo viejo - saldo cero.
			...
			if(movPrestAnterior.getIntItemMovimiento() != null){
				...
				if(ultimoMovimientoInteres != null){
					bdMontoRegistro = 	ultimoMovimientoInteres.getBdMontoSaldo().subtract(expedienteCreditoRef.getBdMontoInteresAtrasado());
				}else{
					bdMontoRegistro = 	expedienteCreditoViejoMovimiento.getBdSaldoInteres().subtract(expedienteCreditoRef.getBdMontoInteresAtrasado());
				}
				...
				if(movPrestAnteriorInteres.getIntItemMovimiento() != null){
					...
					// 9. nuevo estado 
						...
						estadoVigente.setIntCuenta(expRefinanciamiento.getId().getIntItemExpediente());
				...
				if(lstExpedientesMov != null && !lstExpedientesMov.isEmpty()){
					...
					for (Expediente expedienteMov : lstExpedientesMov) {
						if(expedienteMov.getIntParaTipoCreditoCod().compareTo(Constante.PARAM_T_TIPO_CREDITO_REFINANCIADO)==0){
							...
							expIdRefinan.setIntItemDetExpediente(expedienteMov.getId().getIntPersEmpresaPk());
