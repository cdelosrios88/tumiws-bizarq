           ...
           SELECT CONC.PERS_EMPRESACONCILIACION_N_PK pempresaconciliacion,
            ...
            WHERE BCOCTA.PERS_EMPRESA_N_PK = CONC.PERS_EMPRESA_N_PK
                  ...
                  AND (dtBusqFechaDesde IS NULL
                       OR CONC.CORE_FECHACONCILIACION_D >= dtBusqFechaDesde)
                  AND (dtBusqFechaHasta IS NULL
                       OR CONC.CORE_FECHACONCILIACION_D <= dtBusqFechaHasta)
   ...
   BEGIN
      OPEN var_lista FOR
         SELECT PERS_EMPRESACONCILIACION_N_PK pempresaconciliacion,
           ...
           FROM TES_CONCILIACIONRE CONC
          WHERE CONC.CORE_FECHACONCILIA_D =
                   (SELECT MAX (CONC2.CORE_FECHACONCILIA_D)
                      FROM TES_CONCILIACIONRE CONC2
                     WHERE CONC.PERS_EMPRESACONCILIACION_N_PK =
                              CONC2.PERS_EMPRESACONCILIACION_N_PK)
                AND (CONC.PERS_EMPRESACONCILIACION_N_PK =
                        v_empresaconciliacion)
                AND (CONC.PARA_ESTADO_N_COD = V_PARA_ESTADO_N)
                AND (CONC.TESO_ITEMBANCOFONDO_N = V_ITEMBANCOFONDO_N)
                AND (CONC.TESO_ITEMBANCOCUENTA_N = V_ITEMBANCOCUENTA_N);
